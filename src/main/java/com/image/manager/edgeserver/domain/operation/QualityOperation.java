...
public class QualityOperation extends Operation {
    ...
    protected BufferedImage processImage(BufferedImage image, String imageFormat) {
        if(imageFormat.equals("jpg")) {
            try {
                ...
                imageWriteParam.setCompressionQuality((float) compressionRate / 100);
    ...
    protected void validateProperties(BufferedImage image) {
        ...
        Preconditions.checkArgument(compressionRate >=1 && compressionRate <=100, "Compression rate should be inside range [1-100]");
    }
    ...
    public static class Factory implements Operation.Factory {
        ...
        public Operation fromArguments(Map<String, String> arguments) {
            return new QualityOperation(parseNumber(arguments.get(FormatOption.VALUE)));
        }
